...
## Projects
- **AI Reels pipeline (built 14-09-2026, PAUSED):** `skills/daily-reel/` (SKILL.md, `endcard.py`, `publish_media.sh`), `reels/` (prompts, formats, trends, log.csv), config `reels` + `trends`. Waiting on: Bee's Higgsfield plan upgrade (Starter grants only 200 credits/month), then the A/B/C test (Kling 3.0 / Seedance 2.5 / Cinema Studio 3.0 on Bee's street-collision prompt → 3 Metricool drafts), then `reels.enabled` = true with the winning model and `every_n_days` sized to the plan.
